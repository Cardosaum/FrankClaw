...
use frankclaw_core::media::{
    FileScanService, MediaFile, ScanVerdict, mime_for_safe_extension, safe_extension_for_mime,
};
...
impl MediaStore {
    ...
    pub async fn store(
        ...
        if let Some(ref scanner) = self.scanner {
            ...
            info!(
                filename = original_name,
                "file scan clean ({}/{})", verdict.malicious_count, verdict.total_engines
            );
        }
    ...
    pub async fn scan_file(&self, filename: &str, data: &[u8]) -> Result<Option<ScanVerdict>> {
        match self.scanner {
    ...
    pub fn read(&self, id: &MediaId) -> Result<Option<StoredMediaContent>> {
...
mod tests {
    ...
    async fn read_returns_bytes_and_inferred_mime() {
        let temp_dir =
            std::env::temp_dir().join(format!("frankclaw-media-read-{}", uuid::Uuid::new_v4()));
    ...
    async fn read_falls_back_when_metadata_sidecar_is_missing() {
        let temp_dir =
            std::env::temp_dir().join(format!("frankclaw-media-fallback-{}", uuid::Uuid::new_v4()));
        let store = MediaStore::new(temp_dir.clone(), 1024, 1).expect("store should create");
        let media = store
            .store("note.txt", "text/plain", b"hello")
            .await
    ...
    async fn cleanup_removes_sidecar_metadata_with_media_file() {
        let temp_dir =
            std::env::temp_dir().join(format!("frankclaw-media-cleanup-{}", uuid::Uuid::new_v4()));
    ...
    async fn store_with_scanner_rejects_malware() {
        ...
        impl FileScanService for FakeMalwareScanner {
            async fn scan(
                ...
                _filename: &str,
                _data: &[u8],
            ) -> frankclaw_core::error::Result<ScanVerdict> {
                Ok(ScanVerdict {
                    safe: false,
                    malicious_count: 15,
                    total_engines: 72,
        ...
        let temp_dir =
            std::env::temp_dir().join(format!("frankclaw-media-scanner-{}", uuid::Uuid::new_v4()));
        ...
        assert!(
            ...
            "expected malware error, got: {msg}"
        );
    ...
    async fn store_with_scanner_accepts_clean_files() {
        ...
        let temp_dir =
            std::env::temp_dir().join(format!("frankclaw-media-clean-{}", uuid::Uuid::new_v4()));
        let store = MediaStore::new(temp_dir.clone(), 1024, 1)
            .expect("store should create")
    ...
    async fn store_without_scanner_skips_scan() {
        let temp_dir =
            std::env::temp_dir().join(format!("frankclaw-media-noscan-{}", uuid::Uuid::new_v4()));
        let store = MediaStore::new(temp_dir.clone(), 1024, 1).expect("store should create");
