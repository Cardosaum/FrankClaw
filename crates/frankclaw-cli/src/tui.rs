                                ...
                                "think" => {
                                    if args.is_empty() {
                                        let msg = match thinking_budget {
                                            Some(b) => format!("{}: {b}", t!("repl.thinking_budget")),
                                            None => t!("repl.thinking_disabled").to_string(),
                                        };
                                        messages.push(ChatMessage::System(msg));
                                    } else if args == "off" || args == "0" {
                                        thinking_budget = None;
                                        messages.push(ChatMessage::System(t!("repl.thinking_disabled").to_string()));
                                    } else if let Ok(b) = args.parse::<u32>() {
                                        thinking_budget = Some(b);
                                        messages.push(ChatMessage::System(
                                            format!("{}: {b}", t!("repl.thinking_budget_set")),
                                        ));
                                    }
                                }
                                _ => {
                                    messages.push(ChatMessage::System(
                                        format!("{}: /{cmd}", t!("repl.unknown_command")),
                                    ));
                                }
                            }
                            input.clear();
                        ...
                        tokio::spawn(async move {
                            ...
                            match rt.chat(request).await {
                                ...
                                Err(e) => {
                                    forward_handle.await.ok();
                                    let _ = result_tx
                                        .send(StreamResult::Failed(e.to_string()))
                                        .await;
                                }
                            }
                        });
