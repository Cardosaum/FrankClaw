...
mod tests {
    ...
    async fn circuit_breaker_closes_after_successes() {
        ...
        chain
            ...
            .await
            .expect("completion should succeed through circuit breaker");
    ...
    async fn failover_chain_skips_bad_provider_uses_good() {
        ...
        let response = chain
            .complete(request, None)
            .await
            .expect("failover should succeed via second provider");
    ...
    async fn cost_guard_tracks_real_api_usage() {
        ...
        let guard = CostGuard::new(CostGuardConfig {
            max_cost_per_day_cents: Some(10_000), // $100 — won't hit this
        ...
        guard
            .check_allowed()
            .await
            .expect("should be allowed before any calls");
        ...
        guard
            .check_allowed()
            .await
            .expect("should be allowed after one call");
    }
    ...
    async fn cost_guard_hourly_rate_limit() {
        ...
        for _ in 0..3 {
            guard
                .check_allowed()
                .await
                .expect("each of the first three actions should be allowed");
    ...
    async fn cost_tables_have_known_models() {
        ...
        let (input, output) = model_cost("gpt-4o").expect("gpt-4o should have cost data");
