...
fn core_block_to_local(block: &core_canvas::CanvasBlock) -> CanvasBlock {
    let kind: CanvasBlockKind =
        serde_json::from_value(serde_json::Value::String(block.kind.clone()))
...
impl core_canvas::CanvasService for CanvasStore {
    ...
    async fn set(
        &self,
        document: core_canvas::CanvasDocument,
    ...
    async fn patch(
        ...
        let local_blocks: Vec<CanvasBlock> =
            append_blocks.iter().map(core_block_to_local).collect();
...
fn render_markdown(document: &CanvasDocument) -> String {
    ...
    if let Some(session_key) = document
        .session_key
        .as_deref()
