...
pub fn parse_markdown(input: &str) -> MarkdownIR {
            ...
            Event::End(tag_end) => match tag_end {
                TagEnd::Emphasis => {
                    if let Some((start, style)) = pop_style(&mut style_stack, MarkdownStyle::Italic) {
                        if start < text.len() {
                            styles.push(StyleSpan { start, end: text.len(), style });
                        }
                    }
                }
                TagEnd::Strong => {
                ...
                TagEnd::Strikethrough => {
                    if let Some((start, style)) = pop_style(&mut style_stack, MarkdownStyle::Strikethrough) {
                        if start < text.len() {
                            styles.push(StyleSpan { start, end: text.len(), style });
                        }
                    }
                ...
                TagEnd::CodeBlock => {
                    in_code_block = false;
                    if let Some((start, style)) = pop_style(&mut style_stack, MarkdownStyle::CodeBlock) {
                ...
                TagEnd::BlockQuote(_) => {
                    if let Some((start, style)) = pop_style(&mut style_stack, MarkdownStyle::Blockquote) {
                        if start < text.len() {
                            styles.push(StyleSpan { start, end: text.len(), style });
                        }
                    }
                ...
                TagEnd::Link => {
                    if let Some((start, href)) = link_stack.pop() {
                        if start < text.len() {
                            links.push(LinkSpan { start, end: text.len(), href });
                        }
                    }
                }
                TagEnd::List(_) => {
    ...
    MarkdownIR { text, styles, links }
}

/// Pop a specific style from the stack (not necessarily the top, for robustness).
fn pop_style(stack: &mut Vec<(usize, MarkdownStyle)>, target: MarkdownStyle) -> Option<(usize, MarkdownStyle)> {
...
mod tests {
    ...
    fn blockquote() {
        ...
        assert!(ir.styles.iter().any(|s| s.style == MarkdownStyle::Blockquote));
    }
