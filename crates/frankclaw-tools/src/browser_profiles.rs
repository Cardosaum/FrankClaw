...
use frankclaw_core::error::{ConfigValidation, Result};
...
pub fn validate_profile_name(name: &str) -> Result<()> {
    ...
    if name.starts_with('-') || name.ends_with('-') {
        return Err(ConfigValidation {
            msg: "browser profile name must not start or end with a hyphen".to_string(),
        }
        .build());
    }
    if !name
    ...
    {
        return Err(ConfigValidation {
            msg: "browser profile name must contain only lowercase alphanumeric characters and hyphens".to_string(),
        }
        .build());
    }
    Ok(())
...
const PROFILE_COLORS: &[&str] = &[
    "#4A90D9", "#50C878", "#E74C3C", "#F39C12", "#9B59B6", "#1ABC9C", "#E67E22", "#3498DB",
    "#E91E63", "#00BCD4",
];
