...
const SECRET_PATTERNS: &[&str] = &["key", "token", "secret", "password", "credential", "auth"];
...
impl Tool for ConfigGetTool {
    ...
    async fn invoke(&self, args: serde_json::Value, ctx: ToolContext) -> Result<serde_json::Value> {
        let config = ctx.config.as_ref().ok_or_else(|| {
            AgentRuntime {
                msg: "config.get is not available: no config loaded".to_string(),
            }
            .build()
        })?;
...
impl Tool for AgentsListTool {
    ...
    async fn invoke(
        ...
        let config = ctx.config.as_ref().ok_or_else(|| {
            AgentRuntime {
                msg: "agents.list is not available: no config loaded".to_string(),
            }
            .build()
        })?;
...
mod tests {
    ...
    fn navigate_json_path_works() {
        ...
        assert_eq!(
            navigate_json_path(&value, "gateway.port"),
            Some(&serde_json::json!(18789))
        );
        assert_eq!(
            navigate_json_path(&value, "gateway.bind"),
