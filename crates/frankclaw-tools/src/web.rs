...
impl Tool for WebFetchTool {
    ...
    async fn invoke(&self, args: serde_json::Value, ctx: ToolContext) -> Result<serde_json::Value> {
        let url = args
            ...
            .map(str::trim)
            .filter(|v| !v.is_empty())
            .ok_or_else(|| invalid_request_err("web.fetch requires a non-empty url"))?;
        ...
        let fetcher = ctx.fetcher.as_ref().ok_or_else(|| {
            agent_runtime_err("web.fetch is not available: no fetcher service configured")
        })?;
...
impl Tool for WebSearchTool {
    ...
    async fn invoke(
        ...
        let query = args
            ...
            .map(str::trim)
            .filter(|v| !v.is_empty())
            .ok_or_else(|| invalid_request_err("web.search requires a non-empty query"))?;
        ...
        let client = reqwest::Client::builder()
            ...
            .build()
            .map_err(|e| internal_err(format!("failed to build HTTP client: {e}")))?;
        ...
        let response = client
            ...
            .send()
            .await
            .map_err(|e| agent_runtime_err(format!("Brave Search API request failed: {e}")))?;
        ...
        let body: serde_json::Value = response.json().await.map_err(|e| {
            agent_runtime_err(format!("failed to parse Brave Search response: {e}"))
        })?;
